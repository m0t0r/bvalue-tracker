...
- **"¿Cuánto duró?": how long the M7.4 shook, with the fault's own time beside it** (story step after
  ...
  - **The fault's own time is one sentence and one grey bar: about 54 s**, USGS's finite-fault model
    ...
    after ~56 s (up to 180 s in the file) holds ~1% of the moment and does not move the 95% point.
    USGS's own event texts state no duration. It is shown so the reader sees the ground keep moving
    after the fault stops. **Why 54 and not the central 90% (24 s)** (code review, 2026-09-26): the
    model's zero is its hypocentral time, where the modelled rupture starts, so its own answer to "how
    long did the fault move" is the time to 95%; the central 90% was chosen only to compare sources
    whose zeros differ, and nothing is compared now. The first ~30 s release under 5% of the moment and
    are the model's least certain part, which is why the copy says "según el modelo del USGS". Either
    figure sits under the 90 s of shaking, so the step's point does not rest on the choice.
  - **Why no past earthquakes are compared** (owner, 2026-09-26). The first version (PR #65) compared
    *rupture* durations: the M7.4's central 90% (24 s) against Neira 1995 (3 s) and Calima 1995 (2 s),
    from SCARDEC (Vallée & Douet 2016, doi:10.1016/j.pepi.2016.05.012). The figures were right for what
    they measured (recomputed in Python; M6.4–6.5 ruptures of 2–4 s are ordinary, intermediate-depth ones
    short), but in the owner's review "24 s / 3 s / 2 s" read as how long each shook, and so as plainly
    wrong, as it would to any reader: the note under the bars could not undo the bars. **Do not
  ...
  - **Rupture durations for the past events, checked and not used** (2026-09-26), so nobody researches
    ...
    **Global CMT half-durations are not measurements**: GCMT's format document says they are "assumed
    ...
    step in seconds.
  - **Armenia 1999 is not on the step** (owner decision, 2026-09-26). The one inversion
