...
|---|---|---|---|
| 01 | `core` | **Strong — verified** | Admit events through one gate, not three |
...
**Top recommendation: 01.** The only candidate whose failure was reproduced rather
than inferred, the smallest in blast radius, and it *deletes* interface rather than
relocating it — `requireRange` and `requireNum` leave `seiscomp.ts`, which shrinks to
reading HTML shape. It also gives 08 somewhere to put the field table and lets 07
assume its input is plausible.

## The two verified findings

**01 — the CSV door is unguarded.** `requireRange` (`core/seiscomp.ts:59-73`) is applied
...
- 08 buys locality, not leverage — no caller's interface shrinks. Do it after 01.
...
I want to resume at step 3 of /mattpocock-skills:improve-codebase-architecture —
the grilling loop — on candidate 01. Use the mattpocock-skills:codebase-design
vocabulary exactly (module, interface, implementation, depth, seam, adapter,
leverage, locality).

Work in a worktree. Other sessions push to main, so fetch and rebase first.
Start by re-reading candidate 01's files, confirm the finding still holds,
then grill me on it.
```

Swap `01` for whichever candidate you want.
