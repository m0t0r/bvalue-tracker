...
const es = {
  ...
  far: {
    ...
    motion: "Amplitud en Pereira, simplificada",
  ...
  big: {
    ...
    intro: (units: string) =>
      `El rectángulo gris es aquel sismo partido en ${units} puntitos: cada uno es la energía de un evento de M4.0. Elige con qué compararlo.`,
  ...
  swarm: {
    ...
    driftTitle: (km: number) => `${f0(km * 2)} km × ${f0(km * 2)} km alrededor de donde empezó, cada 12 horas.`,
    driftPanel: (when: string, n: number) => `${when} · ${n}`,
  ...
  felt: {
    ...
    tapHint: "toca un día para ver sus eventos",
...
const en: Copy = {
  ...
  far: {
    ...
    motion: "Amplitude in Pereira, simplified",
  ...
  big: {
    ...
    intro: (units) =>
      `The grey rectangle is that earthquake split into ${units} tiny dots: each one is the energy of an M4.0 event. Pick something to compare it with.`,
  ...
  swarm: {
    ...
    driftTitle: (km) => `${f0(km * 2)} km × ${f0(km * 2)} km around where it began, every 12 hours.`,
    driftPanel: (when, n) => `${when} · ${n}`,
  ...
  felt: {
    ...
    tapHint: "tap a day to see its events",
